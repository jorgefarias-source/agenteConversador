...
- **Integração com outros sistemas da Jordão é via API HTTP**: ver `docs/INTEGRACAO.md` — é o documento de referência para conectar PetShop/Mordomê/Cuida ao Conversador (cadastro de tenant, QR do WhatsApp, FAQ, escalonamento, modo piloto). `GET /v1/handoff/pending` ainda não filtra por tenant (pendência registrada lá).
